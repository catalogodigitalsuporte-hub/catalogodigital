...
const CatalogView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCatalogData = async () => {
      ...
      try {
        setLoading(true)
        setError(null)
        const catalogResponse = await lumi.entities.reseller_catalogs.list({
          filter: { catalog_id: catalogId },
          limit: 1
        })
        const catalogData = catalogResponse?.list?.[0]
        if (!catalogData) {
        ...
        const productsResponse = await lumi.entities.catalog_products.list({
          filter: { catalog_id: catalogId },
          sort: { created_at: -1 }
        })
        const productsList = Array.isArray(productsResponse?.list) ? productsResponse.list : []
        setProducts(productsList)
        try {
          await lumi.entities.reseller_catalogs.update(catalogData._id, {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full text-center">
          <LucideIcons.ShoppingBag className="w-16 h-16 text-gray-400 mx-auto mb-4" />
  ...
  return (
    <div className="fixed inset-0 bg-gray-100 flex items-center justify-center z-50 p-0 sm:p-4">
      <div className="w-full h-full sm:w-80 sm:h-[700px] bg-gray-900 sm:rounded-3xl p-0 sm:p-3 shadow-2xl overflow-hidden">
        <div className="hidden sm:block w-full h-6 bg-black rounded-t-2xl relative">
          <div className="absolute top-1 left-1/2 transform -translate-x-1/2 w-20 h-4 bg-gray-900 rounded-full"></div>
        </div>
        <div className="w-full h-full sm:h-[calc(100%-24px)] bg-white sm:rounded-b-2xl overflow-hidden">
            ...
            >
              ...
              >
                <div className="relative">
                  <LucideIcons.ShoppingCart size={24} className="text-white" />
                  {cartItemsCount > 0 && (
                  ...
                  <div className="p-6">
                    ...
                    {cart.length === 0 ? (
                      <div className="text-center py-8">
                        <LucideIcons.ShoppingBag className="w-12 h-12 text-gray-400 mx-auto mb-3" />
